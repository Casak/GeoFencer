...
public class AntennaDataObservableImpl implements AntennaDataObservable {
    ...
    @Override
    public synchronized void removeObserver(LocationInteractor.OnLocationChangedListener observer) {
        mObservers.remove(observer);
    }

    @Override
    public synchronized void passLocation(Point point) {
        notifyObservers(point);
    }

    private void notifyObservers(Point point) {
        for (LocationInteractor.OnLocationChangedListener observer : mObservers) {
    ...
    private static class MockLocationAsyncTask extends AsyncTask<Point, Point, String> {
        AntennaDataObservable listener;
        int timeout;

        public MockLocationAsyncTask(AntennaDataObservable dataProvider, int timeout) {
